...
def insert_claim(subject_name: str, relation: str, object_name: str, source_url: Optional[str] = None, confidence: str = "High", original_text: Optional[str] = None) -> Optional[str]:
    ...
    try:
        ...
        cur.execute(
            ...
            (claim_id, subject_id, relation, object_id, source_id, confidence, original_text)
        )
        conn.commit()
        return claim_id
    except Exception as e:
...
def detect_collision(subject: str, relation: str, object_name: str) -> bool:
    ...
    try:
        ...
        for row in existing_claims:
            ...
            if ("not" in existing_rel and "not" not in new_rel) or ("not" in new_rel and "not" not in existing_rel):
                return True
...
def get_full_graph() -> dict:
    ...
    try:
        ...
        nodes = [{"id": r["id"], "name": r["name"], "type": r["type"]} for r in cur.fetchall()]
        
        cur.execute("SELECT id, subject_id, relation, object_id, confidence FROM claims")
        edges = [{"id": r["id"], "source": r["subject_id"], "target": r["object_id"], "label": r["relation"], "confidence": r["confidence"]} for r in cur.fetchall()]
